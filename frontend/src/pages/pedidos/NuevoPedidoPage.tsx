...
import { z } from 'zod';
import { Plus, Trash2, ArrowLeft, Search, X, Users, List } from 'lucide-react';
import { pedidosService, clientesService, productosService } from '../../services';
import { Spinner, LoadingScreen, Modal } from '../../components/common';
...
function ClienteBuscador({ value, onChange, error }: {
  ...
  const selectedCliente = value ? (clienteActual || value) : null;
  ...
  if (selectedCliente) {
    const nombre = typeof selectedCliente === 'object' ? selectedCliente.nombre : '';
    const documento = typeof selectedCliente === 'object' ? selectedCliente.documento : '';
    return (
      <div>
        <label className="label">Cliente *</label>
        <div className="input flex items-center justify-between bg-primary-50 border-primary-300">
          <span className="text-sm font-medium">{nombre} <span className="text-gray-400">— {documento}</span></span>
          <button type="button" onClick={clear}><X size={14} className="text-gray-400 hover:text-red-500"/></button>
        </div>
